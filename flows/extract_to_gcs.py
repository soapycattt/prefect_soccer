...
TRANSFER_URL = "https://api.github.com/repos/emordonez/transfermarkt-transfers/contents/data"
...
class Crawler():
    ...
    def extract_download_links(self, season_responses_list: list[dict], seasons: list = None): 
        seasons = seasons or self.seasons
        season_leagues = []
    ...
    async def fetch_league(self, download_tuple: tuple):
        ...
        path = os.path.join('data', self.RAW_PATH, season, filename) #data/transfers/raw/1993/something.csv
    ...
    async def run(self):
        ...
        print(f"Completed downloading {len(downloads_tuple)} csv files from {len(self.seasons)} seasons")
...
async def async_write_gcs(bucket, from_paths: list[Path], to_paths: list[Path]):
    ...
    loads = [write_gcs(bucket, from_path, to_path)
